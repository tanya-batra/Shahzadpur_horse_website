...
<main class="container mt-5">
  ...
  <div class="text-center mb-5">
    <h2 class="section-title-modern">{{ $subcategory->name }}</h2>
    <p class="section-subtitle-modern">{{ $subcategory->short_description ?? 'No description available.' }}</p>
  </div>

  <div class="row align-items-center g-5">
    {{-- Left: Subcategory Image --}}
    <div class="col-lg-6" data-aos="fade-right">
      <img src="{{ $subcategory->image ? asset($subcategory->image) : asset('assets/images/placeholder.jpg') }}" 
           alt="{{ $subcategory->name }}" 
           class="img-fluid rounded shadow" style="width:100%; height:auto; object-fit:cover;">
    </div>

    {{-- Right: Subcategory Details --}}
    <div class="col-lg-6" data-aos="fade-left">
      <h3 class="text-accent fw-bold mb-3">{{ $subcategory->name }}</h3>
      <p class="mb-3">{{ $subcategory->description ?? 'No detailed description available for this subcategory.' }}</p>

      {{-- Horses Listing --}}
      <h5 class="fw-bold mb-3">Horses in this Subcategory</h5>

      @forelse($subcategory->horses as $horse)
        <div class="mb-4 p-3 border rounded shadow-sm" data-aos="fade-up">
          <h6 class="fw-bold">{{ $horse->horse_name }}</h6>
          <div class="row g-2 mb-3">
            <div class="col-md-6 col-12"><strong>Age:</strong> {{ $horse->age ?? 'N/A' }}</div>
            <div class="col-md-6 col-12"><strong>Color:</strong> {{ $horse->color ?? 'N/A' }}</div>
            <div class="col-md-6 col-12"><strong>Height:</strong> {{ $horse->height ?? 'N/A' }}</div>
            <div class="col-md-6 col-12"><strong>Other Info:</strong> {{ $horse->other_info ?? '-' }}</div>
          </div>

          {{-- Horse Images (not carousel) --}}
          @if($horse->images->count())
            <div class="row g-2">
              @foreach($horse->images as $img)
                <div class="col-md-4 col-6">
                  <img src="{{ asset($img->image_path) }}" alt="{{ $horse->horse_name }}" class="img-fluid rounded shadow-sm">
                </div>
              @endforeach
            </div>
          @else
            <p class="text-muted">No images available for this horse.</p>
          @endif
        </div>
      @empty
        <p>No horses found in this subcategory.</p>
      @endforelse
    </div>
  </div>
</main>
      <footer class="footer text-white bg-primary-dark mt-5">
        <div class="container">
          <div class="row g-5">
