            ...
            @foreach($category->subcategories as $sub)
                <div class="col-lg-4 col-md-6 col-sm-12">
                    <div class="card shadow h-100">
                        @if($sub->image)
                            <img src="{{ asset($sub->image) }}" class="card-img-top" alt="{{ $sub->name }}">
                        @else
                            <img src="https://via.placeholder.com/400x200?text=No+Image" class="card-img-top" alt="{{ $sub->name }}">
                        @endif
                        <div class="card-body">
                            <h5 class="card-title">{{ $sub->name }}</h5>
                            @if($sub->short_description)
                                <p class="card-text">{{ $sub->short_description }}</p>
                            @endif
<a href="{{ route('subcat.detail', $sub->id) }}" 
   class="btn btn-sm" style="background-color: #152238; color: white;">
   View Detail
</a>
                        </div>
                    </div>
